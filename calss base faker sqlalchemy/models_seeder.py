...
from .all_project_models.user_models import UsrUserMerchant, UsrUser, UsrMerchant, UsrUserGroups, UsrGroupScope, UsrScope, UsrKyc, UsrGroup, \
    UsrSession
...
class UserRelatedSeeder:
    ...
    def create_fake_user(self, number):
        for _ in range(number):
            national_code = str(fake.unique.random_number(digits=10)),
            mobile = fake.phone_number(),
            username = fake.user_name(),
            password = fake.password(),
            kyc_ids = [kyc.id for kyc in self.session.query(UsrKyc.id)]
            try:
                existing_user = self.session.query(UsrUser).filter_by(
                    username=username,
                    mobile=mobile,
    ...
    def create_fake_user_merchant(self, number):
        for _ in range(number):
            ...
            try:
                existing_merchant = self.session.query(UsrMerchant).filter_by(name=name, name_fa=name_fa).one()
                # if you need lot data use below code instance
                # existing_merchant = self.session.query(UsrMerchant).filter(or_(UsrMerchant.name == name,
                #                                                                UsrMerchant.name_fa == name_fa)).first()
                # if the name exists, skip this record and move on to the next one
                continue
            except NoResultFound:
